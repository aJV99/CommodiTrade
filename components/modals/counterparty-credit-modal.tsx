...
export function CounterpartyCreditModal({
  ...
  const [creditLimit, setCreditLimit] = useState<string>(counterparty.creditLimit.toString());
  ...
  const utilization = useMemo(() => {
    ...
    return Math.round((counterparty.creditUsed / counterparty.creditLimit) * 1000) / 10;
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[520px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Recalibrate limits and ratings to reflect the counterparty&apos;s latest performance.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
            ...
            <div className="mt-1 grid gap-1 sm:grid-cols-3">
              <span>Current rating: <strong>{counterparty.rating}</strong></span>
              <span>Limit: <strong>${counterparty.creditLimit.toLocaleString()}</strong></span>
              <span>Used: <strong>${counterparty.creditUsed.toLocaleString()} ({utilization}% )</strong></span>
            </div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="credit-rating">New rating</Label>
              <Select value={rating} onValueChange={value => setRating(value as CreditRating)}>
