...
export type CounterpartyDetail = Awaited<
  ReturnType<typeof getCounterpartyById>
>;
export type CounterpartyPerformance = Awaited<
  ReturnType<typeof getCounterpartyPerformance>
>["performance"];
...
export function CounterpartyDetailClient({
  counterparty,
  performance,
}: CounterpartyDetailClientProps) {
  ...
  const creditUtilization =
    counterparty.creditLimit > 0
      ? Math.min(
          100,
          (counterparty.creditUsed / counterparty.creditLimit) * 100,
        )
      : 0;
  ...
  const commodityData = useMemo(() => {
    ...
    const entries = Object.entries(performance.commodityBreakdown || {}) as [
      string,
      CommodityStats,
    ][];
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Badge
              className={`${typeBadgeColors[counterparty.type]} font-medium`}
            >
          ...
          <div className="space-y-1 text-sm text-slate-600">
            ...
            <p>
              Last trade:{" "}
              {counterparty.lastTradeDate
                ? new Date(counterparty.lastTradeDate).toLocaleDateString()
                : "—"}
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          ...
          <CardContent className="space-y-3 text-sm text-slate-600">
            ...
            <p>
              {formatCurrency(counterparty.creditUsed)} of{" "}
              {formatCurrency(counterparty.creditLimit)} used. Available credit
              is{" "}
              <span className="font-medium text-slate-900">
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm text-slate-600">
            ...
            <p>
              Total contracts traded with volume of{" "}
              {counterparty.totalVolume.toLocaleString()} units.
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-4 text-sm text-slate-600">
            ...
            <div>
              ...
              <p className="mt-1 text-lg font-semibold text-slate-900">
                {performance.totalExecuted.toLocaleString()}
              </p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-slate-500">
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            <div className="space-y-3 rounded-lg border border-slate-200 p-4">
              ...
              <p className="text-sm text-slate-600">
                {performance.totalContracts} contracts worth{" "}
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="mt-1 font-semibold text-slate-900">
                    {performance.completedContracts}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide text-slate-500">
                  ...
                  <p className="mt-1 font-semibold text-slate-900">
                    {performance.cancelledContracts}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide text-slate-500">
            ...
            <div className="rounded-lg border border-slate-200 p-4">
              ...
              {commodityData.length ? (
                ...
                <p className="mt-4 text-sm text-slate-500">
                  No commodity allocation data yet.
                </p>
              )}
            </div>
          </CardContent>
