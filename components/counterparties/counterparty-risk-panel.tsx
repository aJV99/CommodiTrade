...
export type CreditRiskCounterparty = Awaited<
  ReturnType<typeof getCreditRiskCounterparties>
>[number];
      ...
      <CardContent className="space-y-4">
        ...
        {!isLoading && data.length === 0 && (
          <p className="text-sm text-slate-500">
            All counterparties are within healthy utilization thresholds.
          </p>
        )}
        {!isLoading &&
              ...
              <div className="mt-3 space-y-1 text-xs text-slate-500">
                <div className="flex items-center justify-between text-sm text-slate-600">
                  ...
                  <span className="font-medium text-slate-900">
                    ${counterparty.creditUsed.toLocaleString()} / $
                    {counterparty.creditLimit.toLocaleString()}
                  </span>
                </div>
                <Progress
                  value={Math.min(100, counterparty.creditUtilization)}
                />
                <div className="flex items-center justify-between">
                  <span>
                    {counterparty.creditUtilization.toFixed(1)}% utilized
                  </span>
                  <span>
                    Available $
                    {Math.max(0, counterparty.availableCredit).toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
