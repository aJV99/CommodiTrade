...
export async function getDashboardStatistics(userId?: string) {
  try {
    ...
    const [
      ...
      prisma.trade.count({
        where: { ...whereClause, status: TradeStatus.OPEN },
      }),
      prisma.trade.count({
        where: { ...whereClause, status: TradeStatus.EXECUTED },
      }),
    ...
    const portfolioValue =
      (totalTradeValue._sum.totalValue || 0) +
      inventoryValuation.totalMarketValue;
    const portfolioChange = inventoryValuation.unrealizedPnL;
    const portfolioChangePercent =
      inventoryValuation.totalCostValue > 0
    ...
    return {
      ...
      creditUtilization:
        (totalCreditLimit._sum.creditLimit || 0) > 0
...
export async function getCommodityExposure(userId?: string) {
  try {
    ...
    const exposure = Array.from(exposureMap.values()).map((item) => ({
      ...
      totalVolume:
        item.tradeVolume + item.inventoryVolume + item.contractVolume,
    }));

    // Sort by total value descending
    exposure.sort((a, b) => b.totalValue - a.totalValue);

    const totalExposure = exposure.reduce(
    ...
    return {
      exposure: exposure.map((item) => ({
        ...item,
        percentage:
          totalExposure > 0 ? (item.totalValue / totalExposure) * 100 : 0,
      })),
      totalExposure,
    };
  } catch (error) {
...
export async function getSystemHealthMetrics() {
  try {
    const [totalRecords, recentActivity, dataIntegrity] = await Promise.all([
    ...
    return {
      ...
      healthScore:
        dataIntegrity.totalIssues === 0
          ? 100
          : Math.max(0, 100 - dataIntegrity.totalIssues * 10),
