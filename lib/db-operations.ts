...
export async function updateTrade(id: string, data: Partial<Trade>) {
  ...
  if (index !== -1) {
    ...
    if (data.quantity || data.price) {
      mockTrades[index].totalValue = mockTrades[index].quantity * mockTrades[index].price;
    }
...
// Contract operations
export async function createContract(data: Omit<Contract, 'id' | 'totalValue' | 'remaining'>) {
...
export async function updateContract(id: string, data: Partial<Contract>) {
  ...
  if (index !== -1) {
    ...
    if (data.quantity || data.price) {
      mockContracts[index].totalValue = mockContracts[index].quantity * mockContracts[index].price;
    }
    if (data.executed !== undefined) {
      mockContracts[index].remaining = mockContracts[index].quantity - data.executed;
    }
...
export async function updateCounterparty(id: string, data: Partial<Counterparty>) {
