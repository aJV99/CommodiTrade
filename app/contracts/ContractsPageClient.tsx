...
export function ContractsPageClient({
  ...
  const totalExecuted = useMemo(
    () =>
      contracts.reduce(
        (sum, contract) => sum + contract.executed * contract.price,
        0,
      ),
    [contracts],
  );
  const averageContractValue = useMemo(
    () =>
      contracts.length > 0
        ? Math.round(totalContractValue / contracts.length)
        : 0,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-2">
            Manage your commodity purchase and sale contracts
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ${averageContractValue.toLocaleString()}
            </div>
